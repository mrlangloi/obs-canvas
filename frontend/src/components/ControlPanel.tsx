    ...
    */
    ...
    return (
        <aside className={`control-panel ${!showPanel ? 'hidden' : ''}`}>
            <h2>Editing: {displayCard ? displayCard.label : 'None'}</h2>
            
            <div className="control-panel-controls">
                <p>Position: ({displayCard?.position.x || 0}, {displayCard?.position.y || 0})</p>
                
                <p>Rotation: {displayCard?.rotation || 0}°</p>
                <input
                    type="range"
                    min="-360"
                    max="360"
                    value={displayCard?.rotation || 0}
                    onChange={(e) => updateCard(activeCardID!, { rotation: parseInt(e.target.value) })}
                />

                <p>Opacity: {displayCard?.opacity || 100}%</p>
                <input 
                    type="range"
                    min="0"
                    max="100"
                    value={displayCard?.opacity || 100}
